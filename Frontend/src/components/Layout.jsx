    ...
    const stats = useMemo(()=>{
      ...
      const statCard =({title,value,icon})=>{
        ...
        <div className='p-2 sm:p-3 rounded-xl bg-white shadow-sm border-purple-100 hover:shadow-md transition-all duration-300 hover:border-purple-100 group'>
          <div className='flex items-center gap-2'>
            <div className='p-1.5 rounded-lg  bg- bg-gradient-to-br from-fuchsia-500/10 to-purple-500/10 group-hover:from-fuchsia-500/20 group-hover:to-purple-500/20'>
            {icon}
            </div>
      ...
      if(error) return(
        <div className='min-h-screen bg-gray-50 p-6 flex items-center justify-center'>
          <div className='bg-red-50m text-red-600 p-4 rounded-xl border border-red-100 max-w-md'>
  ...
  return (
    <div className='min-h-screen bg-gray-50'>
        ...
        <div className='ml-0 xl:ml-64 lg:ml-64 md:ml-16 pt-16 p-3 sm:p-4 md:p-4 transition-all duration-300'>
          <div className='grid grid-cols1 xl:grid-cols-3 gap-4 sm:gap-6'>
            ...
            <div className='xl:col-span-1 space-y-4 sm:space-y-6'>
              <div className='bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-purple-100'>
                ...
                <div className='grid grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6'>
                  ...
                  <statCard title='complted' value={stats.completedTasks} icon={<circle className='w-3.5 h-3.5 sm:w-4 sm:h-4 text-green-500'/>}/>
                  <statCard title='pending' value={stats.pendingCount} icon={<circle className='w-3.5 h-3.5 sm:w-4 sm:h-4 text-fuchsia-500'/>}/>
                  <statCard title='Completion rate' value={`${stats.completedPercentage}%`} icon={<circle className='w-3.5 h-3.5 sm:w-4 sm:h-4 text-purple-500'/>}/>
                </div>
                <hr className='my-3 sm:my-4 border-purple-100'/>
                <div className='space-y-2 sm:space-y-3'>
                  <div className='flex items-center justify-between text-gray-700'>
                    <span className='text-xs sm:text-sm font-medium flex items-center gap-1.5'>
                      <circle className='w-2 h-2.5 sm:h-3 text-purple-500 fill-purple-500'/>
                      Task Progress
                    </span>
                    <span className='text-xs b-purple=100 text-purple-700 px-1.5 py-0.5 sm:px-2 rounded-full'>{stats.completedTasks}/{stats.totalCount}</span>

                  </div>
                  <div className='relative pt-1'>
                    <div className='flex gap-1.5 items-center'>
                      <div className='flex-1 h-2 sm:h-3 bg-purple-100 rounded-full overflow-hidden'></div>
